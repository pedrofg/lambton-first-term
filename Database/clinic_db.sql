...
--
-- Host: 127.0.0.1
-- Generation Time: May 24, 2017 at 08:19 PM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `QuerryA` ()  BEGIN

SELECT p.id, p.name, p.phone, p.address, Count(p.id) as counter_visits
FROM patient_profile as pp INNER JOIN appointment as ap ON pp.appointment_id = ap.id INNER JOIN patient as p ON ap.patient_id = p.id
where pp.date_time >= DATE_SUB(NOW(), INTERVAL 1 YEAR) GROUP BY p.id HAVING counter_visits >= 2;

END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `QuerryB` ()  NO SQL
SELECT spec.name, COUNT(spec.id) as counter_doctor from doctor as doc INNER JOIN speciality as spec ON doc.speciality_id = spec.id GROUP BY spec.id$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `QuerryC` ()  NO SQL
SELECT doc.id as doctor_id, doc.name as doctor_name, COUNT(app.doctor_id) as patient_counter, MONTH(app.date_time) as month, MONTHNAME(app.date_time) as month_name FROM appointment as app INNER JOIN doctor as doc ON doc.id = app.doctor_id GROUP BY app.doctor_id, month ORDER BY month ASC, patient_counter DESC$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `QuerryD` ()  NO SQL
SELECT doc.id as doctor_id, doc.name as doctor_name, COUNT(app.doctor_id) as patient_counter, MONTH(app.date_time) as month, MONTHNAME(app.date_time) as month_name
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `QuerryE` ()  NO SQL
SELECT doc.id as doctor_id, doc.name as doctor_name, COUNT(app.doctor_id) as appointment_counter, YEAR(app.date_time) as year
FROM appointment as app INNER JOIN doctor as doc ON doc.id = app.doctor_id
where YEAR(app.date_time) = 2017
GROUP BY app.doctor_id
ORDER BY appointment_counter DESC$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `QuerryF` ()  NO SQL
SELECT doc.id as doctor_id, doc.name as doctor_name, COUNT(app.doctor_id) as patient_counter, MONTH(app.date_time) as month, MONTHNAME(app.date_time) as month_name
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `QuerryG` ()  NO SQL
select doc.name as Doctor, count(app.doctor_id) as no_of_times, year(app.date_time) as Year
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `QuerryH` ()  NO SQL
select med.name as Medicine, count(pres.medication_id) as No_of_times_prescribed
from purchase pur 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `QuerryI` ()  NO SQL
select med.name as Medicine_Name , count(pres.medication_id) as No_of_times
from purchase pur 
left join prescription pres on pur.prescription_id = pres.id
left join medication med on pres.medication_id = med.id 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `QuerryJ` ()  NO SQL
select DISTINCT med.name as Medicine_Name_Not_Ordered_In_Last_Month
from medication med 
where med.id not in(
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `QuerryK` ()  NO SQL
select DISTINCT med.name as Medicine_Name_Not_Ordered_In_Last_Year
from medication med 
where med.id not in(
	select pres.medication_id 
	from prescription pres
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `QuerryL` ()  NO SQL
select count(pur.prescription_id), month(pur.date_time) as Month, year(pur.date_time) as Year
from purchase pur
...
(43, '2016-06-02 22:00:53', 2, 76),
(44, '2016-07-28 08:08:27', 2, 36),
(45, '2016-09-05 05:30:19', 2, 8),
(46, '2017-03-11 20:42:30', 2, 32),
...
--
ALTER TABLE `prescription`
  ADD CONSTRAINT `medication_id_fk` FOREIGN KEY (`medication_id`) REFERENCES `medication` (`id`),
...
ALTER TABLE `purchase`
  ...
  ADD CONSTRAINT `prescription_id_fk` FOREIGN KEY (`prescription_id`) REFERENCES `prescription` (`id`);
